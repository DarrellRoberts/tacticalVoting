...
const matchPostcode = async () => {
  try {
    const res = await fetch(`https://api.postcodes.io/postcodes/${searchbar}`)
...
<Text style={styles.paragraph}>{contin ? contin?.parliamentary_constituency: "Constituency not found"}</Text>
