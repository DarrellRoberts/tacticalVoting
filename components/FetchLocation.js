...
const fetchCon = async () => {
    if (location) {
    try {
        const data = fetch(`https://api.postcodes.io/postcodes?lon=${location?.longitude}&lat=${location?.latitude}`)
...
//     fetchLonLat()}, 
//     [])
console.log(`https://api.postcodes.io/postcodes?lon=${location?.longitude}&lat=${location?.latitude}`)
...
return (
    ...
    <View>
        {con ? (
        <View>
        <Text style={styles.text}>{con[0]?.parliamentary_constituency}</Text>
